...
class LanyOcr:
    ...
    def visualize(
        ...
        for idx, result in enumerate(results):
            ...
            points = []
            for index in result.line.sub_rrects[0].points:
                points.extend(index)
            point = ','.join(map(str,points))
            result_write.append(f'{point},{result.text}')
            print(f'Text: "{result.text}" - Score: {result.prob} - Points{points}')
